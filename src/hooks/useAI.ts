...
6. **既知施策の超越**: この業種で一般的に行われている標準施策を understanding で明示した上で、ideas にはその先の施策を提案する。
7. **反直感インサイト**: ideas のうち少なくとも1つは、業界の常識に反するが論理的に正しい施策を含める。なぜ反直感なのに有効かのメカニズムを明記する。
8. **撤退・縮小の選択肢**: 「やめるべきこと」「投資を減らすべきこと」も施策候補に含める。`;
      } else if (dep >= 3) {
        // Standard Deep Mode
        deepExpert =
