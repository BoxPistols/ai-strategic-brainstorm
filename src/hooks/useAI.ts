...
export const useAI = () => {
  ...
  const buildPrompt = (pn: string, form: BrainstormForm, dep: number, sesLabel: string, issueStr: string, proMode: boolean) => {
    ...
    const depthMap: Record<number, { understanding: string; desc: string }> = {
      1: { understanding: '1-2文で核心を指摘', desc: '1文で具体的アクション + 「例: 」で実行イメージ1つ' },
      2: { understanding: '2-3文で現状分析と機会を指摘', desc: '2-3文。①なぜ今やるか ②具体的に何をするか ③「例: 」で現場がイメージできる行動例1つ' },
      3: { understanding: '3-5文の深い分析。市場文脈・根本原因・改善機会を構造的に', desc: '3-5文。①なぜ今 ②何をするか ③どうやるか ④「例: 明日○○に△△を実行」レベルの行動例 ⑤期待効果（定量示唆）' },
      4: { understanding: '5-7文の戦略コンサル級分析。業界構造・競合動向・ユニットエコノミクスを含む', desc: '5-7文。①Why ②What ③How ④「例: 」で現場レベルの行動例2つ ⑤定量効果・優先順位根拠 ⑥注意点・リスク' },
    };
