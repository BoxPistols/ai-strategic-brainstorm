...
export const ISSUE_TPL: Record<string, string[]> = {
  ...
  'marketing': ['認知不足','リード獲得コスト高騰','商談化率低下','ターゲット乖離','メッセージング不明確'],
  'growth': ['新規獲得鈍化','解約・離脱増加','LTV低下','市場飽和','セグメント最適化'],
...
export const EXAMPLE_PRODUCTS: Record<string, string[]> = {
  'product':        ['HR人材サービス', 'ATS（採用管理システム）', 'スカウト自動化ツール', 'エンジニア向け転職アプリ'],
  'marketing':      ['HR人材サービス', 'ITエンジニア向け転職メディア', 'エンジニア採用支援SaaS', 'HR系BtoBコンテンツ'],
  'growth':         ['HR人材サービス', 'エンジニア特化型転職サービス', 'ハイスキル人材プラットフォーム', 'B2B2C採用マーケット'],
  'innovation':     ['HR人材サービス', 'AIマッチングエンジン', 'スキルグラフ×キャリアAI', '採用DXプラットフォーム'],
  'cx':             ['HR人材サービス', 'CA（キャリアアドバイザー）支援ツール', '候補者ポータル', '求人票最適化ツール'],
  'ops':            ['HR人材サービス', 'CRM（Salesforce人材紹介特化）', '求職者フォロー自動化', 'スカウト配信管理'],
  'dx':             ['HR人材サービス', 'CA業務のAI自動化', 'マッチングデータ基盤構築', '採用CRM刷新'],
  'design-system':  ['HR人材サービス', '求人サイト（マルチブランド）', 'エンタープライズHR SaaS', 'モバイル求人アプリ群'],
  'other':          ['HR人材サービス', 'IT人材紹介 事業再設計', '直接採用増加への対応', '人材紹介×AIエージェント活用'],
};

export const GOAL_TEMPLATES: Record<string, string[]> = {
  'product':        ['技術的負債50%削減・デプロイ頻度3倍・P1バグ発生率ゼロ', 'ATS/CRM連携強化・スカウト自動化率50%・マッチング精度向上'],
  'marketing':      ['AI検索流入(AIO)月5,000セッション・コンテンツCV率1.5%・CAC削減40%', 'ハイスキルエンジニア月次新規登録+30%・ターゲット適合率2倍'],
  'growth':         ['月次新規エンジニア登録300名・成約件数+40%・平均フィー維持', 'ハイスキル層（年収600万+）登録比率20%・企業クライアント純増20社・LTV改善'],
  'innovation':     ['AIマッチング精度向上・内定率+20pt・JD最適化自動化率80%', 'エンジニアコミュニティ経由流入確立・新収益モデルPoC完了'],
  'cx':             ['求職者NPS +30以上・内定承諾率75%・紹介→内定率25%', 'CA一人当たり対応品質スコア向上・求職者離脱率半減・面談後継続率80%'],
  'ops':            ['CA一人当たり成約件数+50%・CRM入力工数50%削減・フォローアップ自動化率70%', 'スカウト返信率3%→8%・CA間業務標準化・商談化リードタイム短縮'],
  'dx':             ['CRM活用率80%・データドリブン意思決定の標準化・AI活用業務比率30%', '手動スカウト→AI自動化移行・マッチングデータ基盤構築・レポーティング自動化'],
  'design-system':  ['全プロダクト採用率80%・新機能実装速度2倍・a11y AA準拠100%', 'コンポーネント数200件・デザイン-開発ギャップ解消・ブランド一貫性確保'],
  'other':          ['成約単価維持×成約数+30%・直接採用増加への差別化確立・AI活用で人件費構造改善', '縮小市場での独自ポジション確立・ハイスキル特化への転換・新規収益ライン構築'],
};
...
export const ISSUE_TEMPLATES: Record<string, IssueTemplate[]> = {
  ...
  'marketing': [
    { text: 'ターゲット顧客との認知ギャップ', detail: '想定するターゲット層と実際の問い合わせ・来訪者の属性が乖離している', sub: ['ターゲットのメンタルモデル・行動様式の調査が不十分', 'メッセージが刺さっていない'] },
    { text: 'リード獲得コストの高騰', detail: '広告・施策あたりのCPA（獲得単価）が目標を超過し、ROIが悪化', sub: ['特定チャネルへの依存度が高い', 'オーガニック流入・紹介が頭打ち'] },
    { text: '商談化・転換率の低下', detail: '問い合わせ数は確保できているが、実際の商談・成約につながる率が低い', sub: ['初回接触後のフォローが手薄', 'リードの質と量のバランスが崩れている'] },
  ...
  'growth': [
    ...
    { text: '競合・代替手段の増加', detail: '顧客が競合他社や自社内製化に流れるケースが増え、競争環境が激化', sub: ['自社独自の強みが差別化要因になっていない', '価格・フィーへの値下げ圧力が強まっている'] },
  ],
  'cx': [
  ...
  'innovation': [
    ...
    { text: '市場・競合の変化への対応遅れ', detail: '業界構造が変化しているにもかかわらず、既存事業の延長線上の発想から抜け出せていない', sub: ['情報収集・競合分析の頻度が低い', '変化に対するアクションが後手に回っている'] },
  ],
  'design-system': [
