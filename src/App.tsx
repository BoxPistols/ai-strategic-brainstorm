...
const WelcomeVideoModal = lazy(() =>
  import('./components/remotion/WelcomeVideoModal').then((m) => ({ default: m.WelcomeVideoModal })),
);
...
export default function App() {
  ...
  const { ratio, applyRatio, startDrag, startTouchDrag, isDragging, containerRef, activePreset } =
    usePanelResize();
  ...
  const [showWelcomeVideo, setShowWelcomeVideo] = useState(
    () => !localStorage.getItem('ai-brainstorm-welcomed'),
  );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        else if (showResultsTour) {
          localStorage.setItem('ai-brainstorm-results-toured', '1');
          setShowResultsTour(false);
        } else if (showTour) {
          localStorage.setItem('ai-brainstorm-visited', '1');
          setShowTour(false);
  ...
  useEffect(() => {
    if (
      results &&
      !resultsTourTriggered.current &&
      !localStorage.getItem('ai-brainstorm-results-toured')
    ) {
    ...
    <div className={T.page}>
        ...
        <div ref={containerRef} className="flex flex-col lg:flex-row items-start">
          ...
          >
            <div className={`${T.card} p-4`}>
              ...
              {costWarning && (
                <div
                  className={`mb-3 p-2.5 rounded-lg flex items-start gap-2 border ${
                    costWarning.severity === 'critical'
                  ...
                  ? (fmt) => {
                      ...
                      switch (fmt) {
                        case 'md':
                          ...
                          break;
                        case 'txt':
                          dlFile(mdToTxt(report), `${usedName}_${ts}.txt`, 'text/plain');
                          break;
                        case 'csv':
                          dlFile(
      ...
      {showHelp && (
        <AppHelpModal
          ...
          onStartResultsTour={
            results ? () => requestAnimationFrame(() => setShowResultsTour(true)) : undefined
          }
        />
      )}
      {showPrev && report && (
        ...
        <LogPanel
          ...
          onExportOne={(l) => {
            ...
            dlFile(
              ...
              `brainstorm-${l.projectName || l.id}_${ts}.json`,
              'application/json',
            );
          }}
          onImport={importLogs}
