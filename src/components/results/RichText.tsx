...
export const RichText: React.FC<RichTextProps> = React.memo(({ text }) => {
  ...
  const ri = (str: string): React.ReactNode => {
    ...
    while ((m = rx.exec(str)) !== null) {
      ...
      if (m[1])
        parts.push(
          <strong key={m.index} className="font-semibold text-slate-800 dark:text-slate-200">
            {m[2]}
          </strong>,
        );
      else if (m[3])
        parts.push(
          <em key={m.index} className="italic text-slate-600 dark:text-slate-400">
            {m[4]}
          </em>,
        );
      else if (m[5])
