...
export const HeaderBar: React.FC<HeaderBarProps> = ({
  ...
  const groupedSeeds = useMemo(() => {
    const groups: Array<{
      type: string;
      label: string;
      items: Array<{ idx: number; label: string; desc: string }>;
    }> = [];
    ...
    for (const t of typeOrder) {
      const items = seedScenarios
        .map((s, i) => ({
          idx: i,
          label: s.label,
          desc: s.form.productService,
          type: s.form.sessionType,
        }))
        .filter((s) => s.type === t);
    ...
    <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div
          ...
          onMouseLeave={() => {
            seedTimer.current = setTimeout(() => setSeedOpen(false), 200);
          }}
        >
          <button
          ...
          {seedOpen && (
            <div className="absolute right-0 top-full mt-1 w-80 max-h-[28rem] overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg z-50">
              {groupedSeeds.map((g) => (
                <div key={g.type}>
                  <div
                    className={`sticky top-0 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider ${T.t3} bg-slate-50 dark:bg-slate-800/90 border-b border-slate-200 dark:border-slate-700/50`}
                  >
                    {g.label}
                  </div>
                  {g.items.map((s) => (
