...
export const IssueRow: React.FC<IssueRowProps> = ({
  ...
  const subSuggestions = useMemo(() => getSubIssueSuggestions(issue.text), [issue.text]);
    ...
    <div>
      ...
      {open && (
        <div className={`ml-5 mt-1.5 space-y-1.5 pl-2 border-l ${T.div}`}>
          <input
            value={issue.detail || ''}
            onChange={(e) => onChange(idx, 'detail', e.target.value)}
            placeholder="背景・影響度・定量データ…"
            className={T.inpSm}
          />
          {(issue.sub || []).map((s, si) => (
            <div key={si} className="flex items-center gap-1">
              ...
              <input
                value={s}
                onChange={(e) => onSubChange(idx, si, e.target.value)}
                placeholder="サブ課題…"
                className={`${T.inpSm} flex-1`}
              />
              <button
                onClick={() => onRemoveSub(idx, si)}
                className="p-0.5 text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400"
              >
          ...
          <div className="flex items-center gap-1.5 flex-wrap ml-3">
            ...
            {subSuggestions
              .filter((s) => !existingSubs.has(s))
              .map((s, i) => (
                <button
                  key={i}
                  onClick={() => {
                    onAddSub(idx);
                    setTimeout(() => onSubChange(idx, (issue.sub || []).length, s), 0);
                  }}
