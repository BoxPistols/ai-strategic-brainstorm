...
export const LogPanel: React.FC<LogPanelProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`${T.card} w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl`}
      ...
      >
        ...
        <div className={`px-4 py-2 border-b ${T.div} flex items-center gap-2 flex-wrap text-xs`}>
          ...
          <button
            onClick={() => onSettings({ ...settings, autoSave: !settings.autoSave })}
            className={`flex items-center gap-1 ml-auto text-xs ${T.t2}`}
          >
            {settings.autoSave ? (
        ...
        <div className={`px-4 py-2 border-b ${T.div} flex items-center gap-2 flex-wrap text-xs`}>
          <button
            onClick={onExportAll}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-xs ${T.btnGhost}`}
          >
          ...
          {logs.length > 0 && (
            <button
              onClick={onDeleteAll}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg border text-xs bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-700/40 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 transition ml-auto"
            >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {logs.length === 0 && (
            <p className={`text-xs ${T.t3} text-center py-8`}>ログがありません</p>
          )}
          {logs.map((log) => (
            <div key={log.id} className={`${T.cardFlat} p-3`}>
              ...
              <p className={`text-xs ${T.t2} mb-2 line-clamp-2`}>
                {log.results?.understanding?.slice(0, 120)}…
              </p>
              <div className="flex items-center gap-1.5">
                <button
                  onClick={() => onExportOne(log)}
                  className={`text-xs ${T.accentTxt} flex items-center gap-0.5`}
                >
                  <Download className="w-3 h-3" />
                  JSON
                </button>
                <button
                  ...
                  className="text-xs text-red-500 dark:text-red-400 flex items-center gap-0.5 ml-auto"
                >
                  <Trash2 className="w-3 h-3" />
                  削除
                </button>
