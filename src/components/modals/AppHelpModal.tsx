...
const Section: React.FC<{ title: string; children: React.ReactNode; noBorder?: boolean }> = ({
  title,
  children,
  noBorder,
}) => (
...
const steps = [
  {
    icon: Target,
    label: 'セッションタイプを選択',
    desc: 'マーケティング・グロース・新規事業など目的に合った分析軸を選びます',
  },
  {
    icon: Layers,
    label: 'サービス名・目標・課題を入力',
    desc: '分析対象のサービスと、チームの目標・現状課題を記入します',
  },
  {
    icon: SlidersHorizontal,
    label: '分析深度を選択',
    desc: 'Lite〜High-Classの4段階。深いほど詳細な分析が得られます',
  },
  {
    icon: Sparkles,
    label: '「戦略アイデア生成」で分析開始',
    desc: 'AIが状況を分析し、優先度付きの戦略アイデアを生成します',
  },
  {
    icon: Search,
    label: '深掘り・ブラッシュアップ',
    desc: '気になるアイデアの深掘り分析や、追加指示でのブラッシュアップが可能です',
  },
];
...
const features = [
  {
    icon: FlaskConical,
    label: 'Seed',
    desc: 'サンプルデータを投入して、すぐに動作を試せます',
    tip: 'ヘッダーのSeedボタンから選択',
  },
  {
    icon: Database,
    label: 'ログ',
    desc: 'セッション結果の自動保存・エクスポート・インポート',
    tip: 'ヘッダーのログボタンから管理',
  },
  {
    icon: Eye,
    label: 'プレビュー',
    desc: 'レポートをMarkdown・テキストで出力、印刷にも対応',
    tip: '生成結果のプレビューボタンから表示',
  },
  {
    icon: Columns2,
    label: 'レイアウト',
    desc: '左右パネルの比率をプリセットやドラッグで調整',
    tip: 'ヘッダーのレイアウトプリセットから変更',
  },
  {
    icon: Sun,
    label: 'テーマ',
    desc: 'ダーク / ライトモードを切り替え',
    tip: 'ヘッダーの月/太陽アイコンで切替',
  },
];
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 overscroll-contain">
          ...
          <Section title="分析深度">
            <div className="grid grid-cols-2 gap-2.5">
              <div
                className="p-3.5 rounded-xl bg-white/60 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 backdrop-blur-sm"
                title="APIキーなしで利用可能"
              >
          ...
          {/* Tip */}
          <div
            className="p-3.5 rounded-xl bg-amber-50/80 dark:bg-amber-900/15 border border-amber-200/60 dark:border-amber-700/30 backdrop-blur-sm"
            title="APIキー設定のヒント"
          >
            <p className="text-[13px] text-amber-700 dark:text-amber-300 leading-relaxed">
              <strong>Tip:</strong>{' '}
              APIキーの取得方法やコスト目安は、設定パネル内の「取得方法」ボタンで確認できます。
            </p>
          </div>
        </div>
