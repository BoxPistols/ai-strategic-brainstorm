...
export const SupportPanel: React.FC<Props> = ({ apiKey }) => {
  ...
  return (
    <div className={`${T.card} shadow-2xl flex flex-col h-full overflow-hidden`}>
      ...
      {/* コンテンツ */}
      <div
        className={`flex-1 p-4 ${view === 'ai' ? 'overflow-hidden flex flex-col min-h-0' : 'overflow-y-auto overscroll-contain'}`}
      >
        {view === 'top' && (
        ...
        {view === 'faq' && selectedCategory && (
          <SupportFAQList category={selectedCategory} onAskAI={() => setView('ai')} />
        )}

        {view === 'ai' && <SupportAIChat apiKey={apiKey} />}
      </div>
    </div>
  );
};
