...
export const SupportAIChat: React.FC<Props> = ({ apiKey }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const chatMsgs = [{ role: 'system' as const, content: SUPPORT_SYSTEM_PROMPT }, ...recent];
  ...
  return (
    <div className="flex flex-col min-h-0 h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-3 overscroll-contain">
        {messages.length === 0 && (
          <p className={`text-xs ${T.t3} text-center py-6`}>使い方や機能について質問できます</p>
        )}
        {messages.map((m, i) => (
        ...
        {loading && (
          <div className="flex justify-start">
            <div
              className={`px-3 py-2 rounded-xl rounded-bl-sm bg-slate-100 dark:bg-slate-800 ${T.t3}`}
            >
